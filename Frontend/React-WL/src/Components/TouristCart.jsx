...
//remove done
    ...
    const checkoutOrder = async () => {
        try {
            const token = localStorage.getItem("jwtToken");
            const response = await axios.post("http://localhost:8000/api/tourist/cart/checkout", {}, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            console.error('Error checking out order:', error.response ? error.response.data : error.message);
            setError(error.message);
        }
    };
