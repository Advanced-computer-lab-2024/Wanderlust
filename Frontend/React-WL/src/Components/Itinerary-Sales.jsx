...
const ItinerarySales = () => {
  ...
  const fetchItinerary = async () => {
    try {
      ...
      const filteredItineraries = data.filter(itinerary => 
        itinerary.creator === user._id || 
        itinerary.creator === String(user._id) || 
        itinerary.creatorId === user._id || 
        itinerary.creatorId === String(user._id)
      );
  ...
  const filteredData = selectedMonth === 'All' 
    ...
    : {
        itineraries: itinerary.filter(item => {
          const startDate = new Date(item.timeline.start);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 flex items-center">
          ...
          <div>
            <p className="text-gray-600 text-sm">Avg Itinerary Price</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-auto">
            <thead className="bg-indigo-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Itinerary Name</th>
