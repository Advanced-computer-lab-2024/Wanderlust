...
const TouristProfile = () => {
  ...
  const handlePreferenceTagSelect = (tag) => {
    if (selectedPreferenceTags.includes(tag)) {
      setSelectedPreferenceTags(
        selectedPreferenceTags.filter((t) => t !== tag)
      );
    } else {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (lastPromoDate == today) {
        ...
        await axios.post(
          "http://localhost:8000/api/tourist/receiveBirthdayPromo",
          {},
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
  ...
  const fetchPreferenceTags = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/preferenceTag/getpreferenceTags"
      );
  ...
  const handleSavePreferences = async () => {
    try {
      await axios.put(
        `http://localhost:8000/api/preferenceTag/updateTouristPref/${profile._id}`,
        {
  ...
  const redeemPoints = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/tourist/redeemPoints/${username}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8">
      <div className="max-w-6xl mx-auto px-4 flex justify-between items-center">
        ...
        <div
          className="flex items-center text-blue-600 cursor-pointer ml-auto"
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <StatCard
                  icon={
                    <Award
                      className={`w-8 h-8 ${getBadgeColor(profile.badge)}`}
                    />
                  }
                  title="Rank"
